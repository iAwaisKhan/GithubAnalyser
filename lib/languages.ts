 ...
 *  Weights by byte count when GraphQL data is present (more accurate for polyglots).
 *  Falls back to repo count for REST-only (anonymous) users.
 */
export function aggregateLanguages(
  repos: Array<{ language: string | null; languageBytes?: Record<string, number> }>
): LanguageBreakdown {
  // If any repo has byte-level data, use it for all
  const hasByteData = repos.some((r) => r.languageBytes && Object.keys(r.languageBytes).length > 0);

  const counts: Record<string, number> = {};

  if (hasByteData) {
    // Sum bytes across all repos per language
    for (const repo of repos) {
      if (!repo.languageBytes) continue;
      for (const [lang, bytes] of Object.entries(repo.languageBytes)) {
        counts[lang] = (counts[lang] ?? 0) + bytes;
      }
    }
  } else {
    // Fall back to repo count
    for (const repo of repos) {
...
export function detectStrengths(params: {
  ...
  const hasMobile = topLangs.some((l) => ["Kotlin", "Swift", "Dart"].includes(l));
  ...
  if (hasMobile) strengths.push("Mobile / cross-platform development");

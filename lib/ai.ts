...
async function callClaude(prompt: string, maxTokens = 120): Promise<string> {
  ...
  for (let attempt = 0; attempt <= 2; attempt++) {
    ...
    try {
      ...
      if (!RETRY_STATUSES.has(res.status)) return "";

      // Retryable: wait then retry (unless last attempt)
      if (attempt < 2) {
...
// ── Batched calls (3.1) — reduces 9 API calls to 5 ──────────────────────────

/**
 * Batch all repo reviews in a single prompt instead of 5 separate calls.
 * Returns Record<repoName, review>.
 */
export async function getBatchedAiReviews(repos: RepoInput[]): Promise<Record<string, string>> {
  if (repos.length === 0) return {};

  const repoList = repos
    .map(
      (r, i) =>
        `${i + 1}. Name: ${r.name} | Language: ${r.language ?? "Unknown"} | Stars: ${r.stargazers_count} | Forks: ${r.forks_count} | Description: ${r.description ?? "None"}`
    )
    .join("\n");

  try {
    const text = await callClaude(
      `Review these GitHub repositories for code quality, usefulness, and professionalism.
Return ONLY a JSON array of objects with "name" and "review" keys. One line per repo. No preamble, no markdown.
...
${repoList}`,
    ...
    const parsed: Array<{ name: string; review: string }> = JSON.parse(text);
    return Object.fromEntries(parsed.map((r) => [r.name, r.review]));
  } catch {
    // Fall back to individual calls on parse failure
    return getAiReviews(repos);
  }
}

/**
 * Combine getDeveloperAnalysis + getPersonaInsight into one call (shared input data).
 * Returns { summary, personaInsight }.
 */
export async function getBatchedAnalysisAndPersona(
  analysis: DeveloperAnalysisInput,
  persona: PersonaInsightInput
): Promise<{ summary: string; personaInsight: string }> {
  const FALLBACK_SUMMARY = "Analysis unavailable at the moment.";
  const FALLBACK_PERSONA = `You are ${persona.personaType} — a developer with a unique and evolving technical identity.`;
  ...
  try {
    const text = await callClaude(
      `Analyze this GitHub developer profile and return ONLY a JSON object with exactly two keys:
- "summary": 2-3 professional sentences on their technical profile, potential, and growth areas
- "personaInsight": one punchy sentence starting with "You are ${persona.personaType} —" explaining their identity

No preamble, no markdown, no extra keys.

Profile:
- Languages: ${langList}
- Inferred skills: ${analysis.skills.join(", ") || "N/A"}
- Avg repo quality: ${analysis.avgRepoScore}/100
...
- Top languages: ${persona.topLanguages.slice(0, 3).join(", ")}`,
      350
    );
    ...
    return {
      summary: parsed.summary || FALLBACK_SUMMARY,
      personaInsight: parsed.personaInsight || FALLBACK_PERSONA,
    };
  } catch {
    // Fall back to individual calls
    const [summary, personaInsight] = await Promise.all([
      getDeveloperAnalysis(analysis),
      getPersonaInsight(persona),
    ]);
    return { summary, personaInsight };
  }
}
